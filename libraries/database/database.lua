...
function database:open()
	local db_type = conf.db.type
	if( db_type == 'pgsql' ) then
		db = require("libraries.database.pgsql");
		local ok,err = db:open(
			("user=%s password=%s dbname=%s hostaddr=%s port=%d"):format(
			conf.db.username,
			conf.db.password,
			conf.db.dbname,
			conf.db.hostname,
	...
	elseif( db_type == 'mysql' ) then
		db = require("libraries.database.mysql");
		local ok, err = db:open(
			conf.db.dbname,
			conf.db.username,
			conf.db.password,
	...
	elseif ( db_type == 'sqlite' ) then
		db = require("libraries.database.sqlite");
		local ok,err = db:open()
		if (ok) then
			self.db = db
			return true
...
-- @param input string | array la entrada a sanear
...
local function sanitize(input)
    ...
    elseif (type(input) == "table") then
        ...
        for k,v in pairs(input) do
            clean[k] = database:escape(input[k])
        end
        return clean
    end
    return input
...
local function sanitize_query(query)
	if ( string.find(query, "'") ) then
		return nil, gettext('Invalid query')
	end
